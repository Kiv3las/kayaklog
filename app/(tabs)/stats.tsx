...
import { statsForDays, weekBarData, monthBarData, yearBarData, formatTime, BarDataItem } from '../../lib/stats';
import { isoFromDate, mondayOf, weekRangeLabel, formatMonthYear } from '../../lib/dates';
import { spacing, radius } from '../../constants/theme';

type PeriodState = { type: StatsPeriodType; date: Date };
...
function isCurrentPeriod(state: PeriodState): boolean {
  const now = new Date();
  if (state.type === 'week') return isoFromDate(mondayOf(state.date)) === isoFromDate(mondayOf(now));
  if (state.type === 'month') return getYear(state.date) === getYear(now) && getMonth(state.date) === getMonth(now);
...
function makeStyles(c: Colors) {
  return StyleSheet.create({
    ...
    title: { fontSize: 22, fontWeight: '800', color: c.textPrimary, marginBottom: spacing.md },
    segmented: {
      flexDirection: 'row',
    ...
    navLabel: { fontSize: 15, fontWeight: '700', color: c.textPrimary, textAlign: 'center' },
    currentBadge: {
      marginTop: 4,
    ...
    currentBadgeText: { color: c.success, fontSize: 12, fontWeight: '700' },
    backLink: { color: c.primary, fontSize: 12, marginTop: 4 },
    ...
    statLabel: { fontSize: 12, color: c.textTertiary, marginTop: 2, textAlign: 'center' },
    chartCard: {
      backgroundColor: c.cardBg,
      borderRadius: radius.md,
      padding: spacing.md,
    ...
    noData: { textAlign: 'center', color: c.textTertiary, paddingVertical: 24 },
  });
}
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.navigator}>
          <TouchableOpacity onPress={() => setPeriod((p) => navigate(p, -1))}>
          ...
          <TouchableOpacity onPress={() => !current && setPeriod((p) => navigate(p, 1))} disabled={current}>
            <Ionicons name="chevron-forward" size={24} color={current ? colors.border : colors.primary} />
          </TouchableOpacity>
        </View>

        <View style={styles.grid}>
          <StatCard label={t('stats.km')} value={`${stats.km}`} icon="speedometer-outline" styles={styles} colors={colors} />
          <StatCard label={t('stats.time')} value={formatTime(stats.timeMinutes)} icon="time-outline" styles={styles} colors={colors} />
          <StatCard label={t('stats.laps')} value={`${stats.laps}`} icon="repeat-outline" styles={styles} colors={colors} />
          <StatCard label={t('stats.days')} value={`${stats.days}`} icon="calendar-outline" styles={styles} colors={colors} />
          <StatCard label={t('stats.riversCountries')} value={`${stats.rivers} / ${stats.countries}`} icon="water-outline" styles={styles} colors={colors} />
          <StatCard label={t('stats.avgRating')} value={stats.avgRating > 0 ? `${stats.avgRating} ⭐` : '—'} icon="star-outline" styles={styles} colors={colors} />
        </View>
