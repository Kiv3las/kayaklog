import type { Translations } from './es';

const en: Translations = {
  months: {
    long: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
    short: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
  },
  days: {
    short: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
  },
  tabs: {
    home: 'Home',
    log: 'Log',
    stats: 'Stats',
    rivers: 'My Rivers',
  },
  auth: {
    tagline: 'Your kayak logbook',
    signIn: 'Sign In',
    register: 'Create Account',
    name: 'Your name',
    email: 'Email',
    password: 'Password (min. 6 characters)',
    enterBtn: 'Sign In',
    createBtn: 'Create Account',
    forgotPassword: 'Forgot your password?',
    enterEmailFirst: 'Enter your email first.',
    resetSent: 'We sent you a password reset link. Check your email.',
    confirmEmail: 'Account created. Check your email to confirm it.',
    emailAlreadyRegistered: 'This email already has an account. Sign in or reset your password.',
    resetTitle: 'New password',
    resetSubtitle: 'Create a new password for your account',
    newPassword: 'New password',
    confirmPassword: 'Confirm password',
    passwordTooShort: 'Password must be at least 6 characters',
    passwordsDontMatch: 'Passwords don\'t match',
    updatePassword: 'Update password',
    resetSuccess: 'Password updated',
    resetSuccessDetail: 'Sign in with your new password.',
    resetExpired: 'The link expired or is invalid. Request a new one.',
    timeoutError: 'Connection took too long. Check your network and try again.',
  },
  home: {
    hello: 'Hi, {{name}}',
    yearSubtitle: 'Your year on the water',
    yearStats: '{{km}} km · {{days}} days on the water',
    season: 'Season {{year}}',
    km: 'Kilometers',
    laps: 'Laps',
    time: 'Time',
    rivers: 'Rivers',
    countries: 'Countries',
    days: 'Days',
    lastTrip: 'Last trip',
    newDay: 'New day',
    stats: 'Statistics',
    myRoutes: 'My routes',
    loading: 'Loading...',
  },
  add: {
    newDay: 'New day',
    editDay: 'Edit day',
    cancel: 'Cancel',
    save: 'Save',
    saveChanges: 'Save changes',
    date: 'Date',
    today: 'Today',
    dayNotes: 'Day notes',
    notesPlaceholder: 'How was the session?',
    river: 'River {{n}}',
    riverName: 'Name',
    riverNamePlaceholder: 'River name',
    country: 'Country',
    class: 'Class',
    location: 'Location',
    start: 'Start',
    startSet: 'Start ✓',
    end: 'End',
    endSet: 'End ✓',
    lap: 'Lap {{n}}',
    km: 'Km',
    hours: 'Hours',
    minutes: 'Minutes',
    rating: 'Rating',
    comment: 'Comment',
    optional: 'Optional...',
    addLap: 'Add lap',
    addRiver: 'Add river',
    removeRiver: 'Remove river',
    removeLap: 'Remove lap',
    duplicateLap: 'Duplicate lap',
    section: 'Section',
    sectionAlto: 'Upper',
    sectionMedio: 'Middle',
    sectionBajo: 'Lower',
    sectionTodo: 'Todo',
    sectionPlaceholder: 'Custom...',
    waterLevel: 'Water level',
    flow: 'Flow (m³/s)',
    flowOptional: 'Flow · optional',
    daySummary: '{{laps}} laps · {{km}} km · {{time}}',
  },
  water: {
    bajo: 'Low',
    medio: 'Medium',
    alto: 'High',
    crecida: 'Flood',
  },
  riverDetail: {
    km: 'Km',
    laps: 'Laps',
    time: 'Time',
    hardestClass: 'Top class',
    rating: 'Rating',
    sections: 'Sections',
    waterHighlights: 'Water level',
    mostWater: 'Most water',
    leastWater: 'Least water',
    history: 'Lap history',
    sortDate: 'Date',
    sortWater: 'Water',
    empty: 'No laps logged',
  },
  log: {
    title: 'Log',
    empty: 'No entries in this period',
    deleteTitle: 'Delete this day?',
  },
  stats: {
    title: 'Statistics',
    week: 'Week',
    month: 'Month',
    year: 'Year',
    current: 'Current',
    backToWeek: 'Back to this week',
    backToMonth: 'Back to this month',
    backToYear: 'Back to this year',
    km: 'Kilometers',
    time: 'Time',
    laps: 'Laps',
    days: 'Days',
    riversCountries: 'Rivers / Countries',
    avgRating: 'Average rating',
    kmPerDay: 'Kilometers per day',
    kmPerWeek: 'Kilometers per week',
    kmPerMonth: 'Kilometers per month',
    noData: 'No data in this period',
  },
  rivers: {
    title: 'My rivers',
    empty: 'No rivers in this period',
    km: 'km',
    class: 'Class {{level}}',
    count_one: '{{count}} river',
    count_other: '{{count}} rivers',
  },
  flows: {
    title: 'River flows',
    homeTitle: 'Rivers now',
    riverNow: 'Flow now',
    seeAll: 'See all',
    segMine: 'My rivers',
    segFlows: 'Flows',
    loading: 'Loading flows…',
    error: 'Could not load river flows. Pull to retry.',
    empty: 'No stations available',
    noData: 'No data',
    sampleBanner: 'Sample data — DGA connection is under development',
    updatedAgo: '{{time}} ago',
    justNow: 'just now',
    minutes_one: '{{count}} min',
    minutes_other: '{{count}} min',
    hours_one: '{{count}} hour',
    hours_other: '{{count}} hours',
    days_one: '{{count}} day',
    days_other: '{{count}} days',
    detail: {
      current: 'Current flow',
      chart: 'Last 7 days',
      thresholds: 'Reference levels',
      thresholdBajo: 'Low: below {{value}} m³/s',
      thresholdMedio: 'Medium: {{from}}–{{to}} m³/s',
      thresholdAlto: 'High: {{from}}–{{to}} m³/s',
      thresholdCrecida: 'Flood: above {{value}} m³/s',
      station: 'Station',
      stationName: 'Name',
      code: 'BNA code',
      region: 'Region',
      source: 'Source: DGA hydrometric network (MOP)',
      empty: 'No readings in the last 7 days',
      min: '7d min',
      avg: '7d avg',
      max: '7d max',
    },
  },
  settings: {
    title: 'Settings',
    back: 'Back',
    notifications: 'Notifications',
    dailyReminder: 'Daily reminder',
    permissionDenied: 'Permission denied',
    enableInSystem: 'Enable notifications in System Settings.',
    saveFailed: 'Couldn\'t save',
    tryAgainLater: 'Check your connection and try again.',
    reminderTime: 'Reminder time',
    previewBtn: 'Notification preview',
    notifOnHint: "You'll only receive the reminder on days you haven't logged a session.",
    notifOffHint: "Enable the reminder to receive a daily notification if you haven't logged a session.",
    data: 'Data',
    daysLogged: 'Days logged',
    totalLaps: 'Total laps',
    profile: 'Profile',
    yourName: 'Your name',
    account: 'Account',
    signOut: 'Sign out',
    signOutTitle: 'Sign Out',
    signOutConfirm: 'Are you sure you want to sign out?',
    signOutBtn: 'Sign Out',
    cancelBtn: 'Cancel',
    deleteAccount: 'Delete account',
    deleteAccountTitle: 'Delete account',
    deleteAccountConfirm: 'This permanently deletes your account and all your logs. This cannot be undone.',
    deleteAccountBtn: 'Delete',
    deleteAccountFailed: 'Couldn\'t delete account',
    deleteAccountHint: 'Permanently deletes your account and all your data from the server.',
    deleting: 'Deleting...',
    language: 'Language',
    langAuto: 'Auto (device)',
    langEs: 'Español',
    langEn: 'English',
    appearance: 'Appearance',
    appearanceAuto: 'Auto',
    appearanceLight: 'Light',
    appearanceDark: 'Dark',
  },
  map: {
    title: 'My routes',
    back: 'Back',
    all: 'All',
    satellite: 'Satellite view',
    searchPlaceholder: 'Search place...',
    searchNoResults: 'Place not found',
    start: 'Start',
    end: 'End',
    putIn: 'Put in',
    takeOut: 'Take out',
    coordinates: 'Coordinates',
    copy: 'Copy',
    share: 'Share',
    copied: 'Copied',
    noRoutes: 'No routes with location',
    noRoutesHint: 'Add location when logging a river to see it here',
    routes_one: '{{count}} route',
    routes_other: '{{count}} routes',
    pins_one: '{{count}} pin',
    pins_other: '{{count}} pins',
    inYear: 'in {{year}}',
    inHistory: 'in your history',
    startLabel: 'Start · {{names}}',
    endLabel: 'End · {{names}}',
    routeCount: '{{routes}} · {{pins}} {{period}}',
    pickerTitle: 'Location',
    pickerCancel: 'Cancel',
    pickerDone: 'Done',
    pickerStart: 'Start',
    pickerEnd: 'End',
    pickerHintStart: 'Tap the map to place the start pin · Drag to adjust',
    pickerHintEnd: 'Tap the map to place the end pin · Drag to adjust',
    pickerNoPermission: 'Permission denied',
    pickerEnableLocation: 'Enable location in System Settings.',
  },
  filter: {
    title: 'Filter entries',
    general: 'General',
    allEntries: 'All entries',
    allYear: 'All {{year}}',
    days_one: '{{count}} day',
    days_other: '{{count}} days',
  },
  streak: {
    counter_one: '{{count}}-day streak',
    counter_other: '{{count}}-day streak',
    record: 'Best: {{count}}',
    paddledToday: 'You paddled today!',
    keepStreak: 'Paddle today to keep your streak',
  },
  confirm: {
    cannotUndo: 'This action cannot be undone.',
    cancel: 'Cancel',
    delete: 'Delete',
  },
  notif: {
    title: 'Kayak',
    body: 'Did you paddle today? Log a session',
  },
  autocomplete: {
    placeholder: 'River name',
    detail: 'Class {{level}}',
    trips_one: '{{count}} trip',
    trips_other: '{{count}} trips',
  },
  dayCard: {
    editDay: 'Edit day',
    deleteDay: 'Delete day',
    class: 'Class {{level}}',
  },
  countryPicker: {
    title: 'Select country',
    placeholder: 'Select country',
    search: 'Search country...',
    close: 'Close',
  },
  stars: {
    label_one: '{{count}} star',
    label_other: '{{count}} stars',
  },
  daysOfWeek: {
    veryShort: ['M', 'T', 'W', 'T', 'F', 'S', 'S'],
  },
  achievements: {
    title: 'Achievements',
    unlocked: '{{count}} of {{total}} unlocked',
    records: 'Personal records',
    empty: 'Log trips to unlock achievements',
    homeCard: 'Achievements',
    section: {
      rivers: 'Legendary rivers',
      year: '{{year}} goals',
      allTime: 'Lifetime',
    },
    cat: {
      distance: 'Distance',
      days: 'Consistency',
      rivers: 'Explorer',
      countries: 'Globetrotter',
      streak: 'On a roll',
      class: 'Whitewater',
      river: 'Legendary',
    },
    unit: {
      days_one: '{{count}} day',
      days_other: '{{count}} days',
      rivers_one: '{{count}} river',
      rivers_other: '{{count}} rivers',
      countries_one: '{{count}} country',
      countries_other: '{{count}} countries',
      streak_one: '{{count}} day',
      streak_other: '{{count}} days',
      classLabel: 'Class',
    },
    record: {
      totalKm: 'Total distance',
      longestDay: 'Longest day',
      longestLap: 'Longest lap',
      bestMonth: 'Best month',
      hardestClass: 'Hardest class',
      longestStreak: 'Best streak',
    },
  },
  share: {
    button: 'Share',
    season: 'Share season',
    error: 'Could not share',
    tagline: 'My kayak logbook',
    seasonLabel: 'Season {{year}}',
    statKm: 'km',
    statDays: 'days',
    statRivers: 'rivers',
    statStreak: 'streak',
  },
  newDay: 'New day',
};

export default en;
